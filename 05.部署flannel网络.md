...
``` bash
/kubernetes/network/subnets/10.10.24.0-21
/kubernetes/network/subnets/10.10.200.0-21
/kubernetes/network/subnets/10.10.56.0-21
...
etcdctl \
  ...
  --cert-file=/etc/flanneld/cert/flanneld.pem \
  --key-file=/etc/flanneld/cert/flanneld-key.pem \
  get ${FLANNEL_ETCD_PREFIX}/subnets/10.10.200.0-21
```

输出（结果是部署情况而定，网段可能与下面不一样）：

`{"PublicIP":"192.168.10.245","BackendType":"vxlan","BackendData":{"VtepMAC":"9a:fb:8a:ac:8a:f7"}}`
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh ${node_ip} "/sbin/ip addr show flannel.1|grep -w inet"
  done
```
...
``` bash
>>> 192.168.10.247
    inet 10.10.56.0/32 scope global flannel.1
>>> 192.168.10.246
    inet 10.10.24.0/32 scope global flannel.1
>>> 192.168.10.245
    inet 10.10.200.0/32 scope global flannel.1
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh ${node_ip} "ping -c 1 10.10.56.0"
    ssh ${node_ip} "ping -c 1 10.10.24.0"
    ssh ${node_ip} "ping -c 1 10.10.200.0"
  done
```
